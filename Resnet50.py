...
class Bottleneck(nn.Module):
    def __init__(self, in_channel, out_channel, stride=1, widen_factor=4):
        ...
        self.conv_conv = nn.Conv2d(D, D, 3, stride=stride, padding=1, bias=False)
...
class Resnet50(nn.Module):
    def __init__(self, nc):
        ...
        self.layer1 = self.make_blocks(64, channels[0], 1, blocks[0])
        self.layer2 = self.make_blocks(channels[0], channels[1], 2, blocks[1])
        self.layer3 = self.make_blocks(channels[1], channels[2], 2, blocks[2])
    ...
    def make_blocks(self, in_channel, out_channel, stride, depth):
        stage = []
        for i in range(depth):
            if not i:
                stage.append(Bottleneck(in_channel, out_channel, stride))
            else:
                stage.append(Bottleneck(out_channel, out_channel))
